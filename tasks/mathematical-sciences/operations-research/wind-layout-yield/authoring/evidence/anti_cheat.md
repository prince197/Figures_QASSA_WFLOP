...
|---|---|
| Nop (no files) | `test_outputs_parse` cannot open /root/output/layout.csv, so the reward is 0. |
| Hard-code a layout from memory or the literature (Kusiak-Song or a grid) | This lease, turbine count, spacing and wind rose appear nowhere else. A generic ring or grid layout falls ~2.0% short (the ring_layout rung), and the gate is 0.75%. |
| Random feasible layout, report 24 x free-stream power ("no wake loss") | ~5% shortfall and a yield error of about +19%, so both gates fail. |
| Match yield.json to its own layout without optimising | The yield gate passes, but the layout gate fails because the shortfall is 1.5-5%. |
| Write the model's own coarse-sector prediction | The yield error is 3-15% (sector30_model / sector10_model rungs). |
