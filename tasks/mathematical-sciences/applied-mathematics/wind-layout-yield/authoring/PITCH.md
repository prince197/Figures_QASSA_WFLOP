...
- **The hard part.**
  - Choosing a variance-preserving long-term correction: with noisy references and a windy campaign, regression and analog MCP carry the anomaly into the long term.
...
- **What a wrong answer looks like.** A careful regression-plus-residual MCP that passes leave-one-year-out checks but is +3% high. MCP straight onto the raw reanalysis, or with averaged cups: -2% to -5%. Or the mast record used as the climate: +6 to +12%. Or a layout optimised on an uncorrected, veered rose: about 1% of true power lost.
