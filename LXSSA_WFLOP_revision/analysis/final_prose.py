...
The wind farm layout optimization problem (WFLOP) asks where a given number of turbines should be placed so that as little energy as possible is lost in the wakes, while every turbine stays inside the site and keeps a minimum distance from the others. In this work, we propose LX-SSA-VNS, a two-phase hybrid algorithm for the continuous WFLOP. In the first phase, the Laplacian Salp Swarm Algorithm (LX-SSA), introduced by Solanki and Deep in 2023, explores the layout space for half of the evaluation budget. In the second phase, the basic variable neighborhood search (VNS) refines the best layout found by the swarm by alternating shaking of the whole layout with a complete local search. Wake losses are computed with the Jensen model and direction-dependent Weibull wind data, and the boundary and spacing constraints are handled through a penalized objective. We compare LX-SSA-VNS with LX-SSA, SSA, particle swarm optimization (PSO), differential evolution (DE), VNS and a multistart SLSQP solver on 68 benchmark cases (two wind data sets, three circular farms, 2--15 turbines), using 30 seed-paired runs of 6,030 objective evaluations per method and case, and analyze the results with Friedman, Holm-corrected Wilcoxon and effect-size statistics. LX-SSA-VNS obtains the best average rank (1.89), is significantly better than each of the other six methods and finds a feasible layout in 99.9\% of the runs. An ablation with SSA, LX-SSA and VNS shows that the improvement comes from pairing the swarm phase with VNS, which removes on average 39\% of the wake loss left by the swarm, whereas the Laplace step on its own does not improve on the standard SSA. On the Horns Rev~1 offshore site, the hybrid gives the highest mean energy yield for a 16-turbine block, although no method improves on the installed layout within the budget and only the constraint-aware SLSQP solver finds feasible layouts for the full 80-turbine farm. Finally, we show that the ranking of the algorithms is stable under a different power curve but changes under a Gaussian wake model.
""")
...
\item We carry out a component ablation with SSA, LX-SSA, VNS and LX-SSA-VNS that separates the effects of the swarm phase, the Laplace step and the VNS phase, and we examine how the budget should be split between the two phases (Section~\ref{sec:ablation}).
...
\end{itemize}
SSA, LX-SSA, PSO and DE use our original implementation. We implemented VNS, MS-SLSQP and the hybrid for this study; the hybrid calls the original LX-SSA code in its first phase.
...
In this section, we report 14,280 runs (68 test cases, seven methods and 30 seed-paired runs, all with 6,030 objective calls). We then present the component ablation of the hybrid, including 720 additional runs with other budget splits, and the Horns Rev~1 site case. The detailed results of every case are given in Tables~\ref{tab:res-1-500}--\ref{tab:res-2-1000}.
...
To understand where the improvement of the hybrid comes from, we carry out a component ablation. The hybrid has three ingredients: a swarm phase, the Laplace step inside that swarm phase, and the VNS phase. We therefore compare SSA, LX-SSA, VNS and LX-SSA-VNS on all 68 cases with the same seeds and budget (Table~\ref{tab:ablation} and Fig.~\ref{fig:ablation}). The Friedman test over the four variants is highly significant ($\chi^2_F=103.7$, 3 d.f., $p=2.5\times10^{-22}$), and LX-SSA-VNS has the best average rank (1.50), followed by VNS (2.15), SSA (2.81) and LX-SSA (3.54).

Adding the VNS phase is the most important step. LX-SSA-VNS beats LX-SSA in 43 cases without a single loss, and the mean wake loss falls by 0.43 percentage points. Starting VNS from the food source of LX-SSA, rather than from the best initial point, also helps: LX-SSA-VNS beats VNS in ten cases with $N=5$ to 12 and loses only the two three-turbine cases of the 500-m farm, and the mean wake loss falls by 0.07 percentage points.

The Laplace step on its own, in contrast, does not improve on the standard SSA: without the VNS phase, LX-SSA is worse than SSA in nine cases and never better. The split of the budget between the two phases matters little (Table~\ref{tab:split}). The 50\% split gives the highest mean in eight of the 12 cases, but only one difference is significant (Data Set~I, 1000~m, $N=15$, where the 50\% split is better than the 25\% split, $p_{\rm Holm}=0.024$).

Taken together, the ablation shows that the gain of LX-SSA-VNS comes from combining the swarm exploration phase with a complete VNS refinement phase. The counts against VNS differ slightly from Table~\ref{tab:wtl} because the Holm correction is applied to a different family of comparisons.
...
\section{Limitations}
\label{sec:limits}
Our results should be read within the following limits. The ablation shows that the gain of LX-SSA-VNS comes mainly from the combination of a swarm phase with a complete VNS phase; the Laplace step of LX-SSA does not improve on the standard SSA when it is used alone. The hybrid is significantly better than VNS over the whole benchmark, but in single cases the two differ significantly in only eight of the 68 cases.
...
LX-SSA-VNS obtained the best average rank (1.89), was significantly better than every other method over the whole benchmark, including VNS ($p_{\rm Holm}=0.010$), and returned a feasible layout in 99.9\% of the runs. Compared with LX-SSA alone, it was significantly better in 39 of the 68 cases and never worse. The ablation showed that this gain comes from the two-phase design: the VNS phase removes on average 39\% of the wake loss left by the swarm, and starting VNS from the LX-SSA solution is better than starting it from the best initial point, whereas the Laplace step on its own does not improve on the standard SSA. At the Horns Rev~1 site, the hybrid gave the highest mean energy yield for a 16-turbine block, although no method improved on the installed layout within the budget and only the constraint-aware SLSQP solver found feasible layouts for the full 80-turbine farm. The ranking of the methods was stable under a cubic power curve but changed considerably under a Gaussian wake model, where the hybrid nevertheless kept the best average rank.
