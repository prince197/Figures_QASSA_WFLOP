"""Rebuild the manuscript around the proposed hybrid LX-SSA-VNS (seven methods).
...
def setup_text():
    ...
    reps = [
        ...
        ("so runs with the same seed start from the \\emph{same} initial population for all six methods.",
         "so runs with the same seed start from the \\emph{same} initial population for all seven methods."),
...
\end{itemize}
Hybridization therefore turns LX-SSA from the fifth-ranked into the second-ranked method, but it does not surpass stand-alone VNS at this budget.
...
\begin{itemize}
\item \emph{Hybridization substantially improves LX-SSA.} LX-SSA-VNS is significantly better than LX-SSA in 36 of 68 cases and never worse. It raises LX-SSA from the fifth to the second average rank (2.38) and achieves a feasibility rate of 98.1\%.
