...
def main():
    ...
    C.to_csv("hybrid_case_tests.csv", index=False)
...
""")
    ...
    pd.DataFrame(best_rows).drop_duplicates().to_csv("hybrid_best_layouts_maxN.csv", index=False)
...
""")
    ...
    Sp = pd.concat([pd.read_csv("fresh_hsplit.csv"), G[G.Algorithm == FOCUS]], ignore_index=True)
    lines, split_rows = [], []
    for (ds, r, n), s in Sp.groupby(["Dataset", "Radius", "Turbines"]):
        ...
        ps = []
        for b in ("LXVNS25", "LXVNS75"):
            d = piv[FOCUS] - piv[b]; nz = d[np.abs(d) > 1e-9]
            ps.append(wilcoxon(nz).pvalue if len(nz) else 1.0)
        ph = holm(ps)
        cells = []
        for a in ("LXVNS25", FOCUS, "LXVNS75"):
            v = fm.get(a, np.nan)
            c = "--" if not np.isfinite(v) else f"{v:.1f}"
            if fc.get(a, 0) < 30:
                c += f"$^{{{int(fc.get(a, 0))}}}$"
            cells.append(c)
        best = max(("LXVNS25", FOCUS, "LXVNS75"), key=lambda a: fm.get(a, -np.inf))
...
""")
    ...
    open("hybrid_tables.tex", "w").write("\n".join(tex))
    json.dump(summary, open("hybrid_summary.json", "w"), indent=1, default=float)
