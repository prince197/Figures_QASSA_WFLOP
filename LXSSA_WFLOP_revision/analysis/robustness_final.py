...
LAB = {"LXBV": "LX-SSA-VNS", "LXSSA": "LX-SSA", "SSA": "SSA", "PSO": "PSO", "DE": "DE", "VNS": "VNS", "SLSQP": "MS-SLSQP", "BVNS": "VNS"}
...
if __name__ == "__main__":
    G = pd.concat([pd.read_csv("fresh_grid.csv"), pd.read_csv("fresh_vgrid.csv"), pd.read_csv("fresh_bgrid.csv")],
                  ignore_index=True)
    G = G[G.Algorithm.isin(ALGS)]
    G = G[G.Feasible].reset_index(drop=True)
    with Pool(4) as pool:
        R = pd.DataFrame(pool.map(reval, G.to_dict("records"), chunksize=50))
    G = pd.concat([G, R], axis=1)
    G.drop(columns=["Coordinates", "Curve"]).to_csv("final_reevaluation.csv", index=False)
    out, lines = {}, []
...
\end{table*}
"""
    open("final_robust_table.tex", "w").write(tex)
    import json
    json.dump(out, open("final_robust_summary.json", "w"), indent=1, default=float)
