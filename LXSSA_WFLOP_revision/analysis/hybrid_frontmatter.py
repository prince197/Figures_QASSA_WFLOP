...
The Wind Farm Layout Optimization Problem (WFLOP) is a nonlinear constrained optimization problem in which turbine locations interact through wake-induced velocity deficits. This paper proposes LX-SSA-VNS, a two-phase hybrid for continuous WFLOP. The Laplacian Salp Swarm Algorithm (LX-SSA), previously introduced by Solanki and Deep~\cite{Solanki2023}, explores the layout space during the first half of the evaluation budget. Variable neighbourhood search (VNS) then intensifies the best layout found, using nested turbine-perturbation neighbourhoods and an adaptive local search. Both phases share a penalized objective with Jensen wake interactions, direction-dependent Weibull wind statistics and explicit farm-boundary and minimum-spacing constraints. The hybrid is compared with LX-SSA, SSA, particle swarm optimization (PSO), differential evolution (DE), the original basic VNS (BVNS), a modified stand-alone VNS and a gradient-based multistart SLSQP solver on 68 benchmark cases (two wind data sets, three circular farms, 2--15 turbines). Each comparison uses 30 seed-paired runs at an equal budget of 6,030 objective evaluations and is analyzed with Friedman, Holm-corrected Wilcoxon signed-rank and effect-size statistics, together with feasibility rates, convergence curves and an ablation of the budget split. LX-SSA-VNS attains the second-best average rank (2.74) and a feasibility rate of 98.1\%. It is significantly better than LX-SSA in 35 cases and never worse, and it ranks significantly better than SSA, PSO, DE, LX-SSA and SLSQP. The modified VNS remains the best method (average rank 2.09): it is statistically equivalent to the hybrid in 53 cases and better in 15 dense cases, and the ablation attributes the gain of the hybrid to its VNS phase. The original BVNS ranks third (3.62) and is the only method feasible in every run; the modified VNS outperforms it for layouts with six or more turbines. On the Horns Rev~1 offshore site a similar ordering appears, no method improves on the installed layout within the budget, and only the constraint-aware SLSQP finds feasible layouts for the full 80-turbine farm. The algorithm ordering is insensitive to the power-curve model but sensitive to the wake model, and absolute energy values are strongly model-dependent.
""")
...
\item a controlled comparison of the hybrid with LX-SSA, SSA, PSO, DE, the original basic VNS, a modified stand-alone VNS and a gradient-based multistart SLSQP solver on 68 benchmark cases at equal objective-evaluation budgets, with 30 seed-paired runs per method and case, full statistical analysis (Friedman and Holm-corrected Wilcoxon signed-rank tests, effect sizes), feasibility rates and convergence curves (Section~\ref{sec:results});
\item a comparison of the original and the modified VNS (Section~\ref{sec:bvns}) and an ablation that separates the contributions of the two phases and examines the budget split (Section~\ref{sec:ablation});
